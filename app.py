...
def get_analytics():
    ...
    for i in questions:
        ...
        i['options']= opts
    for i in questions:
        f = mysql_db(mysql,f"SELECT COUNT(*) as c FROM answer where question_id={i['QuestionId']} ","dict")
        total = f[0]['c']
        i['total_responses'] = total
        if i['questiontype_id'] ==1:
            for j in i['options']:
                f = mysql_db(mysql,f"SELECT COUNT(*) as c FROM answer_option where question_option_id={j['Id']} ","dict")
                x = f[0]['c']
                j['percent'] = round(x/total * 100,2)
        else:
            f = mysql_db(mysql,f"SELECT *  FROM answer where question_id={i['QuestionId']} ","dict")
    ...
    d['survey_details'] = f
    d['questions'] =questions
    return render_template('analytics.html',data=d)
...
@app.route("/complete_survey",methods=["GET"])
def complete_survey():
    if authenticate(mysql,session)==False:
        return redirect('/login')
    q=f"Update surveys_table set status=CASE WHEN status !=1 THEN 1 ELSE 2 END where survey_id={request.args['id']}"
    mysql_db(mysql,q,"commit")
    return redirect(url_for('my_surveys'))
